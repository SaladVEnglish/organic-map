...
def count_pattern_occurrences(target_str, pattern):
    ...
    regex = re.compile(
        rf'(?:{escaped_pattern}|\({escaped_pattern}\))(\d*)'
    )
...
def get_valence(formula: str, now_valence: int = 0) -> dict:
    atom_dict = parse_chemical_formula(formula)
    atom_cnt = len(atom_dict)
    res = dict()
    visit = set()
    for i in valence_order.keys():
        if atom_cnt == 1:
            break
        if i in atom_dict:
            res[i] = Fraction(valence_order[i])
...
if __name__ == "__main__":
    print(get_valence(parse_chemical_formula(input())))
